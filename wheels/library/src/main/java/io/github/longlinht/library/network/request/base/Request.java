 ...
 */
package io.github.longlinht.library.network.request.base;

import android.text.TextUtils;

import io.github.longlinht.library.base.util.MD5Util;
import io.github.longlinht.library.network.convert.Converter;

import io.github.longlinht.library.network.OkGo;
import io.github.longlinht.library.network.adapter.AdapterParam;
import io.github.longlinht.library.network.adapter.CacheCall;
import io.github.longlinht.library.network.adapter.Call;
import io.github.longlinht.library.network.adapter.CallAdapter;
import io.github.longlinht.library.network.cache.CacheEntity;
import io.github.longlinht.library.network.cache.CacheMode;
import io.github.longlinht.library.network.cache.policy.CachePolicy;
import io.github.longlinht.library.network.callback.Callback;
import io.github.longlinht.library.network.model.HttpHeaders;
import io.github.longlinht.library.network.model.HttpMethod;
import io.github.longlinht.library.network.model.HttpParams;
import io.github.longlinht.library.network.utils.HttpUtils;
...
public abstract class Request<T, R extends Request> implements Serializable {
    ...
    private static final String SIG = "anbi@2018";
    ...
    protected void sign() {
        ...
        Map<String, List<String>> paramsMap = httpParams.urlParamsMap;

        List<Map.Entry<String, List<String>>> entries =
          new ArrayList<Map.Entry<String, List<String>>>(paramsMap.entrySet());
        ...
        for (Map.Entry<String, List<String>> urlParams : sortedMap.entrySet()) {
            List<String> urlValues = urlParams.getValue();
            for (String value : urlValues) {
                //对参数进行 utf-8 编码,防止头信息传中文
                //String urlValue = URLEncoder.encode(value, "UTF-8");

                if ("sign".equals(urlParams.getKey()))
                    continue;

                sb.append(urlParams.getKey()).append("=").append(value == null ? "" : value).append("&");
            }
        }
        ...
        /*
        try {
            params("before_sign", URLEncoder.encode(sb.toString(), "UTF-8"));
        } catch (UnsupportedEncodingException e) {

        }
        */

        String md5 = MD5Util.MD5(sb.append(SIG).toString());
        params("sign", MD5Util.MD5(md5));
    }
