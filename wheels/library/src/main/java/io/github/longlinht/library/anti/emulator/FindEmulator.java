package io.github.longlinht.library.anti.emulator;

import android.content.Context;
import android.telephony.TelephonyManager;

import io.github.longlinht.library.anti.common.Property;
import io.github.longlinht.library.anti.common.Utilities;
import io.github.longlinht.library.anti.debugger.FindDebugger;
...
public class FindEmulator {
    ...
    public boolean hasQEmuProps(Context context) {
        ...
        for (Property property : known_props) {
            String propertyValue = Utilities.getProp(context, property.name);
            // See if we expected just a non-null
            if ((property.seekValue == null) && (propertyValue != null)) {
                foundProps++;
            }
            // See if we expected a value to seek
            if ((property.seekValue != null) && (propertyValue.indexOf(property.seekValue) != -1)) {
                foundProps++;
            }
