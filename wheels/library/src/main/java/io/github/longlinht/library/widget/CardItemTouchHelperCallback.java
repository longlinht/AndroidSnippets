...
import io.github.longlinht.library.utils.CollectionUtils;
...
public class CardItemTouchHelperCallback<T> extends ItemTouchHelper.Callback {
    ...
    public void setMovementFlags(int flag) {
        mMoveFlag = flag;
    }
    ...
    public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        return makeMovementFlags(dragFlags, mMoveFlag);
    }
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (mListener != null) {
            mListener.onSwiped(viewHolder, direction == ItemTouchHelper.UP ? CardConfig.SWIPED_UP : CardConfig.SWIPED_DOWN);
        }
        // 当没有数据时回调 mListener
        if (adapter.getItemCount() == 2) {
            if (mListener != null) {
                mListener.onNeedPreLoad();
                return;
            }
        }
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder,
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            ...
            Logger.i("dy=" + dY + " threshold=" + getThreshold(recyclerView, viewHolder));
            // ratio 最大为 1 或 -1
            if (ratio > 1) {
                ratio = 1;
            } else if (ratio < -1) {
            ...
            */
            if (mListener != null) {
                if (ratio != 0) {
                    mListener.onSwiping(viewHolder, ratio, dY, ratio < 0 ? CardConfig.SWIPING_UP : CardConfig.SWIPING_DOWN);
                } else {
                    mListener.onSwiping(viewHolder, ratio, dY, CardConfig.SWIPING_NONE);
                }
            }
        }
    }
    ...
    private float getThreshold(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        return recyclerView.getHeight() * getSwipeThreshold(viewHolder) * 5;
    }
