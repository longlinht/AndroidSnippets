...
package io.github.longlinht.library.manager;
...
import io.github.longlinht.library.utils.GsonUtils;
import io.github.longlinht.library.utils.Jsons;
...
public class CacheManager {
	...
	public <T> List<String> getIdList(Class<T> clazz, String group) {
		...
		return sp == null ? null : Jsons.parseJson2List(sp.getString(StringUtil.getTrimedString(group), null), String.class);
	}
	...
	public <T> void saveList(Class<T> clazz, String group, LinkedHashMap<String, T> map, int start, int pageSize) {
		...
		if (StringUtil.isNotEmpty(group, true)) {
			...
			//id列表
			List<String> idList = Jsons.parseJson2List(sp.getString(group, null), String.class);
			if (idList == null) {
			...
			editor.remove(group).putString(group, GsonUtils.toJson(idList)).commit();
	...
	public <T> void save(Class<T> clazz, T data, String id, String group) {
		...
		if (sp != null) {
			...
			if (idList.contains(id) == false) {
				...
				sp.edit().remove(group).putString(group, GsonUtils.toJson(idList)).commit();
			}
		}
	}
