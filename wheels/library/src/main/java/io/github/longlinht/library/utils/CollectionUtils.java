...
import io.github.longlinht.library.guava.Preconditions;
...
public class CollectionUtils {
    ...
    public static <K, V> Map<K, V> dict(@NonNull Pair<K, V>... kvPair) {
        ...
        for (Pair<K, V> pair : kvPair) {
            result.put(Preconditions.checkNotNull(pair.first), pair.second);
        }
    ...
    public static <K, V> Pair<K, V> pair(@NonNull K first, V second) {
        return Pair.create(Preconditions.checkNotNull(first), second);
    }
}
